...
def print_graph():
    ...
    # Read data from CSV file
    with open('/home/pi/Documents/Capacitor_Ripple_Simulated_Voltage_10.9.csv', 'r') as file:
        csv_reader = csv.reader(file)
...
#    print(meas_avg)
    ...
    plt.title('Capacitor Ripple Voltage STM32')
    plt.xlabel('Time (s)')
    ...
    # Shows graph, pauses execution for 0.5s, allows for graph to be redrawn
    plt.draw()
    plt.pause(0.5)
    plt.clf()

# define the serial port information - adjusted to use rPi GPIO for UART
...
# Sample voltage 'samps' times and print to screen
samps = 5
for x in range(samps):

    # handle the case, triggers the Bluepill to sample the voltage
    ser.write('s'.encode())
